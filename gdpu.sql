create database my_IT_system collate Chinese_PRC_CI_AS
go

use my_IT_system
go

grant connect on database :: my_IT_system to dbo
go

grant view any column encryption key definition, view any column master key definition on database :: my_IT_system to [public]
go

create table AAS.Activity
(
    ID               int identity
        constraint PK_Activity
            primary key,
    Name             nchar(100),
    OrganizationName nchar(100),
    time             nchar(100),
    MaterialsID      int,
    SiteID           int,
    ExpenditureID    int,
...
exec sp_addextendedproperty 'MS_Description', '活动调用人员', 'SCHEMA', 'AAS', 'TABLE', 'Activity', 'COLUMN', 'GroupID'
go

create table AAS.Expenditure
(
    ID           int identity
    ...
    SubmitTime   varchar(max),
    state        varchar(max)
)
go

create table tmp.[Group]
(
    ID   int identity
        constraint PK_Group
            primary key,
...
create table tmp.GroupNode
(
    ID       int identity
        constraint PK_GroupNode
            primary key,
    MemberID int
)
go

create table CIES.Label
(
    PostID int        not null
        constraint PK_Label
            primary key,
...
create table AAS.Materials
(
    ID                int identity
        constraint PK_Materials
            primary key,
    MaterialsList     nchar(1000),
    Borrower          int,
    time              nchar(200),
    BorrowingPurposes nchar(1000)
)
go

create table AMS.Members
(
    ID           int identity
...
create table tmp.MessageBoxs
(
    ID         int identity
        constraint PK_MessageBoxs
            primary key,
    MessageBox int
)
go
...
create table AAS.Site
(
    ID                  int identity
        constraint PK_Site
            primary key,
    ApplicantSite       nchar(100),
    Borrower            nchar(100),
    time                nchar(100),
    ApplicationPurposes nchar(1000)
)
go
